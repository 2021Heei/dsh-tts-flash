 ...
 *
 * Adapted from @haoku123/dsh-voice (MIT): src/segmenter.ts
 */
...
export function plainText(text: string): string {
  return String(text)
    ...
    .replace(/^\d+\.\s+/gm, '')
    .replace(/<\/?[a-zA-Z][^>]*>/g, ' ')
    // Catch-all: paired markers above miss patterns split across streaming
    // deltas (e.g. '**' landing in two chunks). None of these characters ever
    // belong in spoken text, so strip the strays unconditionally.
    .replace(/[*_~`#>|]+/g, '')
    // emoji & pictographs: engines voice them as garbage ('✅' → 播放按钮)
    .replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{2190}-\u{21FF}\u{2300}-\u{23FF}\u{2500}-\u{25FF}\u{FE00}-\u{FE0F}\u{200D}]/gu, '')
}
